...
import com.mgtv.tv.lib.core.StringUtils;
...
public abstract class Request<T> implements Comparable<Request<T>> {
     ...
     * 进行域名替换的正则表达式
     */
    private static final String REGEX = "^https?://[0-9a-z.]*";
    ...
    /**
     * 设置缓存时间，当 responseTime + mCompleteCacheTime < currentTime < responseTime + mCachePeriod 时，
     * 请求数据先读缓存，同时后台刷新数据
     */
    private long mCachePeriod;

    /**
     * 设置完全缓存时间，当 responseTime + mCompleteCacheTime > currentTime时，
     * 在这个时间段内去请求数据不会在后台刷新数据
     */
    private long mCompleteCacheTime;
    ...
    public final int getConnectTimeoutMs() {
        return mRetryPolicy.getConnectTimeoutMs();
    }
    ...
    public final int getReadTimeoutMs() {
        return mRetryPolicy.getReadTimeoutMs();
    }
    ...
    public void setRequestDomain(String domain) {
        if (StringUtils.isStringEmpty(mUrl)) {
            return;
        }
        Pattern pattern = Pattern.compile(REGEX);
        Matcher matcher = pattern.matcher(mUrl);
        if (matcher.find()) {
            String oldDomain = matcher.group();
            mUrl = mUrl.replace(oldDomain, domain);
        }
    }
    ...
    public void setCompleteCacheTime(long completeCacheTime) {
        this.mCompleteCacheTime = completeCacheTime;
    }

    public long getCompleteCacheTime() {
        return this.mCompleteCacheTime;
    }

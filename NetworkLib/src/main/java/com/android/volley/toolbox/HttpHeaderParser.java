...
public class HttpHeaderParser {
    ...
    public static Cache.Entry parseCacheHeaders(Request request, NetworkResponse response) {
        ...
        // exist and Expires
        // is more restrictive.
        // NOTE(xianggengong):以服务端的响应头中的缓存信息作为缓存策略
        // if (hasCacheControl) {
        ...
        //NOTE(xianggengping):自定义缓存策略
        final long cacheHitButRefreshed = request.getCompleteCacheTime(); // cache will be hit, but also refreshed on background
        final long cacheExpired = now + request.getCachePeriod(); // this cache entry expires completely
